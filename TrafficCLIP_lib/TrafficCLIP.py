...
from kornia.feature.dedode.transformer.layers.block import attn_bias_cache
...
class Transformer_on_visual(nn.Module):
    ...
    def forward(self, x, feature_list,attn_mask=None):
        idx = 0
        out_tokens = []
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,features_list=None):
        ...
        # [8,3,640,640]
        x = self.conv1(x)  # shape = [*, width, grid, grid] #[8,1024,45,45]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] #[8,2025,1024]
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1)  # shape = [*, grid ** 2 + 1, width]  #[8,2026,1024]
        ...
        x = x + self.positional_embedding.to(x.dtype) # [8,2026,1024]
        x = self.ln_pre(x)
...
class TrafficCLIP(nn.Module):
    ...
    def encode_image(self, image,features_list=None):
        return self.visual(image.type(self.dtype),features_list)
    ...
    def encode_text_learn(self, prompts, tokenized_prompts):
        ...
        #x = self.transformer([x,deep_compound_text,0])
